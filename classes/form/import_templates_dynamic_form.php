...
/**
 * Upload a learning plan template CSV for preview.
 *
 * @package    local_dimensions
 * @copyright  2026 Anderson Blaine
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class import_templates_dynamic_form extends \core_form\dynamic_form {
    /**
     * The context the templates would be imported into (system or a course category).
     *
     * @return \context
     */
    ...
    /**
     * Only template managers, and only in system or course-category context, may import.
     *
     * @return void
     */
    ...
    /**
     * Form fields: the CSV file, encoding/delimiter, and the update-existing toggle.
     *
     * @return void
     */
    public function definition() {
        ...
        $mform->addElement(
            'filepicker',
            'importfile',
            get_string('central_plans_import_file', 'local_dimensions'),
            null,
        ...
        $mform->addElement(
            'select',
            'delimiter',
            get_string('central_plans_import_delimiter', 'local_dimensions'),
        ...
        $mform->addElement(
            'select',
            'encoding',
            get_string('central_plans_import_encoding', 'local_dimensions'),
        ...
        $mform->addElement(
            'advcheckbox',
            'updateexisting',
            get_string('central_plans_import_updateexisting', 'local_dimensions')
        );
        $mform->setType('updateexisting', PARAM_BOOL);
        $mform->setDefault('updateexisting', 0);
        $mform->addHelpButton('updateexisting', 'central_plans_import_updateexisting', 'local_dimensions');
    }
    ...
    /**
     * Hand the draft file and the parse settings back to the caller. Writes nothing.
     *
     * @return array{draftitemid: int, contextid: int, delimiter: string, encoding: string, updateexisting: int}
     */
    public function process_dynamic_submission() {
        $data = $this->get_data();
        return [
            'draftitemid' => (int) $data->importfile,
            'contextid' => (int) $this->get_context_for_dynamic_submission()->id,
            'delimiter' => (string) $data->delimiter,
            'encoding' => (string) $data->encoding,
            'updateexisting' => empty($data->updateexisting) ? 0 : 1,
        ];
    }
    ...
    public function validation($data, $files) {
        ...
        $text = self::read_uploaded_csv((int) ($data['importfile'] ?? 0));
        if (trim($text) === '') {
            $errors['importfile'] = get_string('central_plans_import_invalidfile', 'local_dimensions');
            return $errors;
        }
        ...
        if ($parsed['error'] !== '') {
            $errors['importfile'] = $parsed['error'];
        } else if (empty($parsed['templates'])) {
            $errors['importfile'] = get_string('central_plans_import_notemplaterow', 'local_dimensions');
        }
        return $errors;
    }
     ...
     */
    public static function read_uploaded_csv(int $draftid): string {
        global $USER;
