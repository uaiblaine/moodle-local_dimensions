...
controles — o `competency_links.js` tem **837** e entrega o resto.
...
- **AMD:** [`competency_links.js`](../../../amd/src/central/competency_links.js) (837) · [`course_datasource.js`](../../../amd/src/central/course_datasource.js) (81, datasource do autocomplete) · usa `errors.js` (`notifyError`)
...
>   `159a800` (**2026-06-29**); um `git log -- amd/src/central/competency_links.js` mostra **seis**
>   commits depois disso: `fb8c725`, `93e4f69`, `d7578b3`, `7902bd8`, `c10acd0`, `e0fe81d`. O
>   `fb8c725` (contagem de cursos atualizada no lugar) é o que costuma escapar da lista.
...
| `MOD.LINKS-COUNT-REFRESH` | `[sem rótulo]` | efeito de fechamento | `competency_links.js:828-835` → `structure.js:908-920` | `onClose(count)` | é o `fb8c725`, e a claim **procede**. O modal conta `state.rowsEl.children.length` (`js:832` — cada filho do contêiner é **um** card) e devolve; `updateCourseCount` grava em `row.dataset.courses`, repinta o detalhe só se a linha ainda for a ativa (`:913`) e **pisca** a linha (`:919`). **Sem reload do pane** — seleção e expansão da árvore sobrevivem. `count` pode ser `null` (modal fechado antes do `shown`), e o handler sai calado (`:909-911`) |

## Casca do modal (Mustache — as 6 refs que já estavam certas)

| ID | Rótulo | Tipo | Origem | Dados | Regra / notas |
| --- | --- | --- | --- | --- | --- |
| `MOD.LINKS-TITLE` | Cursos e atividades — {nome} | título | `competency_links.js:730` (str), `:760` (`Modal.create`) | str `central_links_title`, `$a` = nome | `core/modal` **puro**, sem `footer` no config — o 7º do censo do IMP-06. **`large: true`** (`:760`) é o único ajuste de largura que este modal tem hoje. `setRemoveOnClose(true)` em `:761` |
| `MOD.LINKS-ROOT` | `[sem rótulo]` | região/raiz | `competency_links.mustache:32` | `data-region="competency-links"` · `.local-dimensions-central-links` | os dois listeners delegados (click e change) pousam aqui (`js:812-824`), **não** no root do modal |
...
| `MOD.LINKS-TOAST` | `[sem rótulo]` | feedback | `competency_links.js:805` (região), `:802` (`ModalEvents.shown`) | `addToastRegion(modal.getBody()[0])` | **5** `addToast` próprios: `courseremoved` (`:561`), `activityadded` (`:584`), `activityremoved` (`:614`), `courseadded` (`:662`), `saved` (`:818`) — mais o de rede do `notifyError`. Ver a seção do toast abaixo: este arquivo é o caso de referência do padrão |

## O picker de adicionar curso

| ID | Rótulo | Tipo | Origem | Dados | Regra / notas |
| --- | --- | --- | --- | --- | --- |
| `MOD.LINKS-ADD-ENH` | `[sem rótulo]` | fiação | `competency_links.js:715-720` (`bindPicker`) | `enhance(selector, false, DATASOURCE, placeholder)` | `false` = **single-select**. Chamado no `shown` (`:825`) — o `enhance` do core resolve por `document.querySelector`, então antes do `modal.show()` não acharia nada |
| `MOD.LINKS-ADD-RESET` | `[sem rótulo]` | re-render | `competency_links.js:663-665` | `Templates.replaceNodeContents(addsel.parentElement, state.addshtml, '')` | um single-select do `core/form-autocomplete` **não tem API de limpar**: a casa re-renderiza o `<div class="mb-3">` inteiro a partir do HTML guardado no `shown` (`:811`) e re-faz o `bindPicker`. É por isso que o `state.addsel` é **re-buscado** em `:716` a cada bind |
| `MOD.LINKS-ADD-EXCL` | `[sem rótulo]` | exclusão | `course_datasource.js:70-72` | `data-exclude` (CSV de ids) | lido via `element.dataset` **a cada busca** (`processResults`), nunca por jQuery `.data()` — que cachearia. O `state.excluded` é um `Set` mantido nos três pontos que mexem na lista: `loadCourses` (`js:456`), `onAddCourse` (`:660`) e `removeCourse` (`:558`) |
...
| `MOD.LINKS-CARD-REMOVE` | Remover curso | botão | `competency_links.js:402-404` (`iconButton`, `:97-112`) | `data-action="remove-course"` · `fa fa-trash` | `title` + `.sr-only`. `removeCourse` (`js:542-562`): confirm `deleteCancelPromise` → WS → tira do `excluded` → remove o card → **toast** (`:561`, é o `e0fe81d`). Só sai quando `course.canmanage` |
| `MOD.LINKS-CARD-OUTCOME` | Outcome: | select | `competency_links.js:406-416` | `data-role="course-outcome"` · `name="course-outcome"` | str `central_links_outcomeprefix`. As opções vêm de `state.courseoutcomes`, passado pelo **servidor** via `opts`. Salva no `change` (`js:813-824`) → `saveOutcome` (`:624-638`) → toast "Salvo" + flash. `disabled` quando `!canmanage` |
...
| `MOD.LINKS-ACTS` | `[sem rótulo]` | contêiner preguiçoso | `competency_links.js:424-428` | `data-role="activities"` · `data-loaded="0"` · nasce `hidden` | `.local-dimensions-central-links-acts` (`styles.css:5507-5511`) — a indentação e a borda que põem as atividades **dentro** do card. O `data-loaded` é a memória da carga: `toggleCourse` só chama `loadActivities` quando `!== '1'` (`js:527`), e `addModule`/`removeModule` o zeram (`:581`, `:612`) para forçar a releitura |
...
| `MOD.LINKS-ACTSEARCH-LIST` | `[sem rótulo]` | dropdown | `competency_links.js:296-299`, render `:301-328` | `data-role="activity-search-list"` · nasce `hidden` | abre no `focus` **e** no `input` (`:330-331`), fecha no `Escape` (`:332-336`) e no clique fora — e o clique fora é varrido no **topo** do `onClick` delegado (`js:678-685`), antes de qualquer roteamento, para todo `activity-search` que não contenha o alvo. `styles.css:5590-5597` |
| `MOD.LINKS-ACTSEARCH-ITEM` | {nome} + {tipo} | botão | `competency_links.js:311-324` | `data-action="add-module"` · `data-cmid` | `addModule` (`js:572-585`): WS → zera `data-loaded` → recarrega → **pisca a linha nova** (`:583`, `flash`, `:178-186`) → toast. O flash mira `[data-cmid="…"]` **depois** do reload, porque o nó velho já morreu |
...
| `MOD.LINKS-ACT-REMOVE` | Remover atividade | botão | `competency_links.js:238-240` (`iconButton`) | `data-action="remove-module"` · `fa fa-times` | **✕, não 🗑** — o trash é do curso, o times é da atividade: a diferença de glifo é a diferença de escopo. `removeModule` (`js:594-615`): confirm → WS → recarrega o contêiner do curso (`:610-613`) → toast. Só com `module.canmanage` |
| `MOD.LINKS-ACT-OUTCOME` | `[sem rótulo]` | select | `competency_links.js:245-248` | `data-role="module-outcome"` · `name="module-outcome"` | opções de `state.moduleoutcomes` — **vetor diferente** do curso, e é por isso que o `saveOutcome` tem dois ramos (`js:626-637`) e dois WS. `aria-label` = str `central_links_outcome` |
...
`competency_links.js:805` chama `addToastRegion(modal.getBody()[0])` no `ModalEvents.shown`
(`:802`), com o motivo escrito no comentário logo acima (`:803-804`). É um dos **4** pontos do
...
**Este modal é o que mais depende disso:** ele tem **5** `addToast` próprios — `courseremoved` (`js:561`),
`activityadded` (`:584`), `activityremoved` (`:614`), `courseadded` (`:662`) e `saved` (`:818`) —,
mais o de rede que o `notifyError` levanta. Nenhuma dessas ações fecha o diálogo, então **todas** precisam de
confirmação visível no lugar. Quatro delas também **piscam** o elemento afetado (`flash`,
`js:178-186`), que é o outro metade do par: o toast diz *o quê*, o flash diz *onde*.
...
| `local_dimensions_link_competency_course` | `:245` | `js:652` | devolve **o curso já montado**, e é por isso que o `onAddCourse` consegue anexar o card sem reler a lista |
| `local_dimensions_unlink_competency_course` | `:253` | `js:555` | |
| `local_dimensions_set_course_link_outcome` | `:261` | `js:629` | |
| `local_dimensions_get_competency_module_links` | `:269` | `js:475` | **uma** leitura devolve `linked` + `available` + `canmanage` |
| `local_dimensions_link_competency_module` | `:277` | `js:576` | |
| `local_dimensions_unlink_competency_module` | `:285` | `js:606` | |
| `local_dimensions_set_module_link_outcome` | `:293` | `js:634` | |
...
| --- | --- | --- |
| `fb8c725` | contagem de cursos atualizada no lugar, sem reload da árvore | **sim** — `js:828-835` → `structure.js:908-920` |
...
| `c10acd0` | lote de strings | **sim** — **23** `getString` num `Promise.all` (`js:731-755`) e **23** `labels[…]` no `state`; o `updateCourseMeta` deixou de ser `async` |
...
| `e0fe81d` | toast ao remover curso | **sim** — `js:561` |
...
> `grep -rnoE 'IMP-[0-9]{2}' docs/design-kit/` devolve `IMP-03`, `IMP-05`, `IMP-06`, `IMP-10` e
> `IMP-11`, e nenhum deles é o expandir. No `mod-participants`, que é a **primeira** tela a carregar
...
**O que fica registrado como não resolvido:** o `large: true` de hoje (`js:760`) e a classe
