...
| `TRK-CARD-NOCOMPLETION` | Completion disabled. | muted text | `progress_card_body.mustache:174-180` | `^enabled` | course has completion tracking switched off; gated on `^locked` too (the overlay already explains an empty card) **and** on `^activity` — a single untracked activity still gets named, see `TRK-CARD-SINGLE` |
...
| `TRK-CARD-MODE` | — | server flag | `calculator::resolve_card_shape()` | `cardmode` (`activity` / `section` / `timeline`) | one resolver, called by both `get_course_progress` and `get_competency_courses`, first match wins: course format `singleactivity` (mirrors `format_singleactivity::get_main_activity()` — format option `activitytype`, section 0 only, first module of that type; deliberately **not** `core_courseformat\main_activity_interface`, which arrived in Moodle 5.1 and this plugin supports 4.5 up) **or** exactly one trackable, visible module → `activity`; else exactly one qualifying (visible, user-visible-or-restricted) section → `section`; else → `timeline`. The course-level lock takes precedence over all three and is resolved separately |
| `TRK-CARD-SINGLE` | activity name | compact state strip | `progress_card_body.mustache:144-169` | `activity` `{cmid, name, url, completed, tracked}` | replaces the timeline entirely (`{{#activity}}` short-circuits it) when `cardmode` is `activity`; a state marker (check/circle) renders only when `tracked` — a single activity with completion tracking off still shows its name and a "Go to activity" link with no marker or state text, the case that used to render nothing |
| `TRK-CARD-SECTION` | section name | compact state strip | `progress_card_body.mustache:190-202` | `section` `{name, hasownname, url}`, `sectionpercentage` (client-derived in `competency_view.js:200-202`) | renders when `cardmode` is `section`: a plain bordered circle carrying the section's own percentage (no SVG arc — that belongs to `TRK-TL-RING`), the section's name only when `hasownname`, and the call to action `access_content` ("Access content"), landing on the section |
| — | — | — | `competency_view.js:197` | `data.istimeline = (data.cardmode === 'timeline')` | the client-derived `issinglesection` flag (`sections.length === 1`) is **gone** — `cardmode` decides server-side before the client ever sees the payload. A genuine one-section course now always arrives as `cardmode: section`, so the timeline's own single-row rendering path is no longer reachable: `istimeline` (and so `TRK-TL-*`) is never true for a course with exactly one qualifying section |
