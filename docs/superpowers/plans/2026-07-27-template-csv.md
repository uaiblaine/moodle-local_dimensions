...
- [ ] **Step 7: Write the front end.** Lift `triggerDownload`/`makeSpinner` into `download.js` with no ARIA change — the defect the kit records at `fwk-structures.md:98-102` does not exist at HEAD — and give the export loader span an accessible name while dropping its `d-inline-flex`, which beats `[hidden]` on Bootstrap 4.
